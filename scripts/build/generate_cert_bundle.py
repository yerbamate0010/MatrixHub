...
certs_dir = Path("tools/ssl_certs")
...
def download_cacert_file(source):
    ...
    if response.status_code == 200:
        ...
        status('Failed to fetch the certificate bundle (HTTP %d).' % response.status_code)
        return False
...
def main():
    ...
    try:
        ...
        if (cert_source == "mozilla" or cert_source == "adafruit" or cert_source == "adafruit-full"):
            ok = download_cacert_file(cert_source)
            cacert_path = os.path.join(certs_dir, "cacert.pem")
            if not ok:
                # Allow offline builds if a previous cacert.pem exists.
                if os.path.isfile(cacert_path):
                    status('Using existing certificate bundle: %s' % cacert_path)
                else:
                    raise InputError('Certificate bundle download failed and %s does not exist' % cacert_path)
            bundle.add_from_file(cacert_path)
