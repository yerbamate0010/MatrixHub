...
const String PsychicRequest::getFilename()
{
    ...
    const String contentDispositionHeader = this->hasHeader("Content-Disposition")
        ? this->header("Content-Disposition")
        : String();
    const String filename = PsychicUploadUtils::resolveFilename(contentDispositionHeader,
                                                                getParam("_filename"),
                                                                this->uri());
    if (filename != "")
        return filename;
...
{
    return PsychicUploadUtils::parseContentDisposition(this->header("Content-Disposition"));
}
...
esp_err_t PsychicRequest::loadBody()
{
    ...
    if (remaining > this->server()->maxRequestBodySize) {
        ESP_LOGE(PH_TAG, "Body too large (%u bytes). Max: %lu", remaining, this->server()->maxRequestBodySize);
        return ESP_FAIL;
    }

    // Allocate in PSRAM (SPIRAM)
    this->_body = (char*)heap_caps_malloc(remaining + 1, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
    if (this->_body == NULL) {
        ESP_LOGE(PH_TAG, "Failed to allocate PSRAM for body (%d bytes)", remaining);
        return ESP_FAIL;
    }
    ...
    while (remaining > 0)
    {
        int received = httpd_req_recv(this->_req, this->_body + offset, remaining);

        if (received == HTTPD_SOCK_ERR_TIMEOUT)
        {
            continue;
        }
        else if (received == HTTPD_SOCK_ERR_FAIL || received <= 0)
        {
...
const String PsychicRequest::header(const char *name)
{
    ...
    if (header_len)
    {
        char *header = (char *)heap_caps_malloc(header_len + 1, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
        if (header == NULL) {
            ESP_LOGE(PH_TAG, "Failed to allocate memory for header %s", name);
            return "";
        }
        httpd_req_get_hdr_value_str(this->_req, name, header, header_len + 1);
...
const char* PsychicRequest::body() const
{
    return this->_body ? (const char*)this->_body : "";
}

size_t PsychicRequest::bodyLength() const
{
...
void PsychicRequest::loadParams()
{
    ...
    if (query_len)
    {
        char *query = (char *)heap_caps_malloc(query_len + 1, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
        if (query == NULL) {
            ESP_LOGE(PH_TAG, "Failed to allocate memory for query string");
        } else {
            httpd_req_get_url_query_str(_req, query, query_len + 1);
            _query.clear();
            _query.concat(query);
    ...
    {
        if (this->_body != NULL) {
            _addParams(String(this->_body), true);
        }
    }
}
...
const String PsychicRequest::getSessionKey(const String &key)
{
    if (this->_session == NULL)
        return "";
    SessionEntry* entry = this->_session->find(key);
    if (entry != nullptr)
...
{
    if (this->_session != NULL)
        this->_session->insert(key, value);
}
...
bool PsychicRequest::authenticate(const char *username, const char *password)
{
    if (hasHeader("Authorization"))
    {
        ...
        if (authReq.startsWith("Basic"))
        {
            ...
            snprintf(toencode, toencodeLen + 1, "%s:%s", username, password);
...
const String PsychicRequest::_getRandomHexString()
{
    ...
    for (i = 0; i < 4; i++)
    {
        snprintf(buffer + (i * 8), sizeof(buffer) - (i * 8), "%08lx", (unsigned long int)esp_random());
    }
