...
esp_err_t PsychicEventSourceResponse::send() {
  ...
  // Pre-calculate header size to avoid reallocations
  size_t estimatedSize = 128; // Base HTTP headers
  for (HTTPHeader header : DefaultHeaders::Instance().getHeaders()) {
    estimatedSize += strlen(header.field) + strlen(header.value) + 4;
  }
...
String generateEventMessage(const char *message, const char *event, uint32_t id, uint32_t reconnect) {
  // Pre-calculate approximate size to avoid multiple reallocations
  size_t estimatedSize = 16; // Base overhead for "data: \r\n\r\n"
  if (reconnect) estimatedSize += 20; // "retry: NNNNNNNNNN\r\n"
  if (id) estimatedSize += 20;        // "id: NNNNNNNNNN\r\n"
  if (event) estimatedSize += strlen(event) + 10; // "event: ...\r\n"
  if (message) estimatedSize += strlen(message) + 8; // "data: ...\r\n"
  
  String ev;
  ev.reserve(estimatedSize);
