  ...
  _onClose(NULL)
{
  ...
  // Hardening: some handlers (JSON parsing, logging, auth wrappers) can be stack-heavy.
  // Increase httpd task stack to avoid stack canary resets under debug builds.
  // NOTE: This is in BYTES (ESP-IDF httpd_config_t), unlike FreeRTOS task stack depth.
...
void PsychicHttpServer::closeCallback(httpd_handle_t hd, int sockfd)
{
  ...
  if (client != NULL)
  {
    ...
    ESP_LOGD(PH_TAG, "No client record %d", sockfd);
...
void PsychicHttpServer::addClient(PsychicClient *client) {
  if (xSemaphoreTake(_clientMutex, portMAX_DELAY)) {
...
void PsychicHttpServer::applyToAllClients(std::function<void(PsychicClient*)> fn) {
  if (xSemaphoreTake(_clientMutex, portMAX_DELAY)) {
    for (auto *client : _clients) {
      fn(client);
    }
...
String urlDecode(const char* encoded)
{
  ...
  // Pre-allocate result string (decoded is always <= encoded length)
  String output;
  if (!output.reserve(length)) {
    return "";
  }
