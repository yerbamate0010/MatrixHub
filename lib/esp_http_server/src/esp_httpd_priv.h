...
#define HTTPD_MALLOC(size)      malloc(size)
#define HTTPD_CALLOC(n, size)   calloc(n, size)
#define HTTPD_REALLOC(ptr, size) realloc(ptr, size)
#define HTTPD_FREE(ptr)            heap_caps_free(ptr)

static inline char *httpd_strdup(const char *s) {
    if (!s) return NULL;
    size_t len = strlen(s) + 1;
    char *new_s = (char *)HTTPD_MALLOC(len);
    if (new_s) {
        memcpy(new_s, s, len);
    }
    return new_s;
