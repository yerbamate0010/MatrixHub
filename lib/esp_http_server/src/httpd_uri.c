                                     ...
                                     const httpd_uri_t *uri_handler)
{
    ...
    for (int i = 0; i < hd->config.max_uri_handlers; i++) {
        if (hd->hd_calls[i] == NULL) {
            ...
            hd->hd_calls[i] = HTTPD_MALLOC(sizeof(httpd_uri_t));
            if (hd->hd_calls[i] == NULL) {
            ...
            /* Copy URI string */
            hd->hd_calls[i]->uri = httpd_strdup(uri_handler->uri);
            if (hd->hd_calls[i]->uri == NULL) {
                /* Failed to allocate memory */
                HTTPD_FREE(hd->hd_calls[i]);
                return ESP_ERR_HTTPD_ALLOC_MEM;
            }
            ...
            if (uri_handler->supported_subprotocol) {
                hd->hd_calls[i]->supported_subprotocol = httpd_strdup(uri_handler->supported_subprotocol);
                if (hd->hd_calls[i]->supported_subprotocol == NULL) {
                    /* Failed to allocate memory */
                    HTTPD_FREE((void *)hd->hd_calls[i]->uri);
                    HTTPD_FREE(hd->hd_calls[i]);
                    return ESP_ERR_HTTPD_ALLOC_MEM;
                }
            } else {
                                       ...
                                       const char *uri, httpd_method_t method)
{
    ...
    for (int i = 0; i < hd->config.max_uri_handlers; i++) {
        ...
        if ((hd->hd_calls[i]->method == method) &&       // First match methods
            ...
            HTTPD_FREE((char*)hd->hd_calls[i]->uri);
            HTTPD_FREE(hd->hd_calls[i]);
            hd->hd_calls[i] = NULL;
