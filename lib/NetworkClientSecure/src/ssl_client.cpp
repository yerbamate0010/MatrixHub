...
// Guard disabled manually to work with ESP-IDF + Arduino
...
#define ROE(x, msg)                                   \
  ...
  if (ret < 0) {
    return cleanup_and_fail(handle_error(ret));
  }
  ...
  if (insecure) {
    ...
    return cleanup_and_fail(handle_error(MBEDTLS_ERR_SSL_FEATURE_UNAVAILABLE));
  } else {
    return cleanup_and_fail(-1);
  }
...
int ssl_starttls_handshake(sslclient_context *ssl_client) {
  ...
  while ((ret = mbedtls_ssl_handshake(&ssl_client->ssl_ctx)) != 0) {
    ...
    network_client_secure_yield_wait(2);
  }
  ...
  if ((flags = mbedtls_ssl_get_verify_result(&ssl_client->ssl_ctx)) != 0) {
    ...
    return handle_error(MBEDTLS_ERR_X509_CERT_VERIFY_FAILED);
  } else {
...
int data_to_read(sslclient_context *ssl_client) {
  int ret;
  int res = mbedtls_ssl_get_bytes_avail(&ssl_client->ssl_ctx);
  if (res > 0) {
    return res;
  }

  const int net_ready = peek_net_receive(ssl_client, 0);
  if (net_ready <= 0) {
    return net_ready;
  }
...
int peek_net_receive(sslclient_context *ssl_client, int timeout) {
  ...
  ret = ret == MBEDTLS_NET_POLL_READ ? 1 : ret;
#else
