...
// Integrate with Project Logging System
#undef LOG_TAG
#define LOG_TAG "USBHID"
...
#define log_e(format, ...) LOGE(format, ##__VA_ARGS__)
#define log_w(format, ...) LOGW(format, ##__VA_ARGS__)
#define log_i(format, ...) LOGI(format, ##__VA_ARGS__)
#define log_d(format, ...) LOGD(format, ##__VA_ARGS__)
#define log_v(format, ...) LOG::Logging::log(ESP_LOG_VERBOSE, LOG_TAG, format, ##__VA_ARGS__)
...
// Optimization: Reduced from 10 to 4. We only use Mouse + Keyboard usually.
#define USB_HID_DEVICES_MAX 4
...
static bool tinyusb_enable_hid_device(uint16_t descriptor_len, USBHIDDevice *device) {
  ...
  log_d("Device[%d] len: %d", tinyusb_loaded_hid_devices_num - 1, descriptor_len);
  return true;
}
...
static bool tinyusb_load_enabled_hid_devices() {
  ...
  // Optimization: Allocate descriptor in PSRAM if available
  tinyusb_hid_device_descriptor = (uint8_t *)heap_caps_malloc(tinyusb_hid_device_descriptor_len, MALLOC_CAP_SPIRAM);
  if (tinyusb_hid_device_descriptor == NULL) {
