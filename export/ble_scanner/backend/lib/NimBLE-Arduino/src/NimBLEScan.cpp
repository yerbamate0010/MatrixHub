...
NimBLEScan::NimBLEScan()
    : m_pScanCallbacks{&defaultScanCallbacks},
      ...
      m_pTaskData{nullptr},
      m_maxResults{0xFF} {
    m_scanResultLock = xSemaphoreCreateMutex();
}
...
NimBLEScan::~NimBLEScan() {
    ...
    if (m_pTempDevice) delete m_pTempDevice;
    if (m_scanResultLock) vSemaphoreDelete(m_scanResultLock);
}
...
# endif
            ...
            // Optimisation: Reuse a temporary device object if we are not storing results (maxResults == 0)
            // and we rely on callbacks only. This avoids large heap fragmentation for long scans.
            // We only do this for passive scans (no scan response merging) or extended advertising,
            // or if we simply don't care about merging (which is implied if we don't store results).
            if (pScan->m_maxResults == 0 /* && pScan->m_scanParams.passive */) {
                 if (!pScan->m_pTempDevice) {
                    pScan->m_pTempDevice = new NimBLEAdvertisedDevice(event, event_type);
                    // Pre-allocate payload buffer to prevent fragmentation from resizes
                    // Max legacy BLE payload = 31 bytes adv + 31 bytes scan response = 62 bytes
                    pScan->m_pTempDevice->m_payload.reserve(64);
                 } else {
                    pScan->m_pTempDevice->set(event, event_type);
                 }
                 advertisedDevice = pScan->m_pTempDevice;
                 
                 // Skip duplicate check from vector (controller handles duplicates if enabled)
                 
                 // Fire callbacks
                 // Note: we don't support onDiscovered vs onResult distinction for temp device 
                 // as we don't track state. We just fire both or Result.
                 // Legacy behavior: fire onDiscovered then onResult.
...
# endif
            ...
            if (advertisedDevice == nullptr) {
                ...
                if (isLegacyAdv && event_type == BLE_HCI_ADV_RPT_EVTYPE_SCAN_RSP) {
                    NIMBLE_LOGD(LOG_TAG, "Scan response without advertisement: %s", advertisedAddress.toString().c_str());
                }

                advertisedDevice = new NimBLEAdvertisedDevice(event, event_type);
                if (advertisedDevice) {
                    xSemaphoreTake(pScan->m_scanResultLock, portMAX_DELAY);
                    pScan->m_scanResults.m_deviceVec.push_back(advertisedDevice);
                    xSemaphoreGive(pScan->m_scanResultLock);
                    NIMBLE_LOGD(LOG_TAG, "New advertiser: %s", advertisedAddress.toString().c_str());
                }
            } else {
                advertisedDevice->update(event, event_type);
                if (isLegacyAdv) {
                    if (event_type == BLE_HCI_ADV_RPT_EVTYPE_SCAN_RSP) {
                        NIMBLE_LOGD(LOG_TAG, "Scan response from: %s", advertisedAddress.toString().c_str());
                    } else {
                        NIMBLE_LOGD(LOG_TAG, "Duplicate; updated: %s", advertisedAddress.toString().c_str());
                    }
                }
            }
...
void NimBLEScan::erase(const NimBLEAddress& address) {
    const NimBLEAdvertisedDevice* dev = m_scanResults.getDevice(address);
    if (dev) {
        erase(dev);
    }
}
...
void NimBLEScan::erase(const NimBLEAdvertisedDevice* device) {
    ...
    xSemaphoreTake(m_scanResultLock, portMAX_DELAY);
    ...
    xSemaphoreGive(m_scanResultLock);
}
...
NimBLEScanResults NimBLEScan::getResults() {
    xSemaphoreTake(m_scanResultLock, portMAX_DELAY);
    NimBLEScanResults results = m_scanResults;
    xSemaphoreGive(m_scanResultLock);
    return results;
}
