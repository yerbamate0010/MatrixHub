...
/**
 * MatrixHub: DRAM Optimization Overrides
 * We #undef first to avoid redefinition warnings from sdkconfig.h
 */
...
/** @brief MatrixHub: Increased to 2 (PSRAM available) - allows one active + one pending/stuck connection */
#define CONFIG_BT_NIMBLE_MAX_CONNECTIONS 2

/** @brief MatrixHub: Reduced to 4096 to save DRAM */
#define CONFIG_BT_NIMBLE_HOST_TASK_STACK_SIZE 4096

/** @brief MatrixHub: Disabled CENTRAL role to save DRAM/Flash */
#define CONFIG_BT_NIMBLE_ROLE_CENTRAL 0

/** @brief MatrixHub: Increased to 256 (PSRAM available) - better throughput for history sync */
#define CONFIG_BT_NIMBLE_ATT_PREFERRED_MTU 256

/** @brief MatrixHub: Disabled timestamps to save 4 bytes per attribute value */
...
/** @brief MatrixHub: Disabled NimBLE debug logs (saves ~32kB flash) */
#define CONFIG_BT_NIMBLE_LOG_LEVEL 0

/** @brief MatrixHub: Disabled NimBLE CPP wrapper debug logs */
...
/** @brief MatrixHub: Disabled CLIENT role - we only use PERIPHERAL + OBSERVER.
 *  Saves ~7kB flash and heap for client connection management.
 */
// #define CONFIG_BT_NIMBLE_ROLE_CENTRAL 0
...
/** @brief MatrixHub: Increased to 4 bonded devices (PSRAM available) */
#define CONFIG_BT_NIMBLE_MAX_BONDS 3

/** @brief MatrixHub: Reduced from 8 to 4 CCCD subscriptions */
...
/** @brief MatrixHub: Use external PSRAM for the NimBLE host to save internal DRAM */
...
/**
 * @brief MatrixHub: Use mbedtls instead of tinycrypt.
 * @details Saves ~8kB flash since mbedTLS is already used for Telegram TLS.
 */
